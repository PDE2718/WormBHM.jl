@kwdef struct Kagome <: BH_Parameters{true,3,4,4}
...
function wl_size(H::Kagome)::NTuple{3,Int}
    return (Int(H.Lx), Int(H.Ly), 3)
end
function get_nbs(H::Kagome, i::Integer)::NTuple{4,Int}
    Lx = Int(H.Lx)
    Ly = Int(H.Ly)
...
get_hps(H::Kagome, i)::NTuple{4,Int} = get_nbs(H::Kagome, i)
# function control_site(H::Kagome, i::Integer, j::Integer)::Int
#     nbs = get_nbs(H, i)
#     if j == nbs[1]
#         return nbs[2]
#     elseif j == nbs[2]
...
function diagE(H::Kagome, i::Integer, ni::StateType, njs::NTuple{4,StateType})::f64
    return ni * (0.5 * H.U * (ni - 1) - H.μ + H.V * sum(njs))
end

function bond_weight(H::Kagome, i, j)::f64
    return (j ∈ get_hps(H, i)) ? H.J : 0.0
    # return H.J
end
function simple_measure_names(::Type{Kagome})
    return (:E, :E², :N, :N²,
        :N₁, :N₁², :N₂, :N₂², :N₃, :N₃²,
        :K, :U, :μ, :V,
        :W₁², :W₂², :W₃², :W₁W₂, :W₂W₃, :W₃W₁,
...
function simple_measure!(m, x::Wsheet{3}, H::Kagome)::Nothing
    U = μ = V = 0.0
    Ns = H.Lx * H.Ly
    N1 = N2 = N3 = 0
    W1 = W2 = W3 = 0
