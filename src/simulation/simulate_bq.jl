@generated function simulate_bq!(
    x::Wsheet{Nw},
    H::Ham,
        ...
        while time_ns() < t_limit
            ...
            $(
                if T_DensityMapBoson ≠ Nothing
                    quote
                        ρmap = density_map_boson.ρ
                        wl_boson = H.bosonwl
                        @inbounds for i ∈ eachindex(ρmap, wl_boson)
                            ρmap[i] += first(integrated_density(Val(1), wl_boson[i]))
                        end
