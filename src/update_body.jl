@generated function worm_cycle!(
    ...
    is_KCM_Kagome::Bool = (Ham == KCM_Kagome)
    ...
    quote
        ...
        @inbounds for _ ∈ 1:n_cycle
            ...
            if dice < cycle_accum_prob.AP_move_worm         # [MOVE_WORM]
                # println("   moving...")
                D = randsign()
                if D * δ == i8(-1)
                ...
                # # after move, if (KCM & _at_nb_kink), then record the neighbor
                # $(if is_KCM_Kagome quote
                #     v_control = 
                # end end)

                @goto CYCLE_START🔁

            elseif dice < cycle_accum_prob.AP_insert_kink  # [INSERT_KINK]
                # println("   inserting...")
                if loc ≠ _at_free
                    @goto CYCLE_START🔁
                end
                ...
                $(
                    if is_KCM_Kagome
                        quote
                            lk = x[control_site(H, i, j)]
                            nk = lk[vindex(lk, head.t)].n_L
                            if nk == StateType(0)
                                @goto CYCLE_START🔁
                            end
                        end
                ...
                Wk = max(nj, nmid)
                P_acc = 2 * $zhps * Wk * bond_weight(H, i, j) * update_consts.P_del2ins
                if metro(P_acc)
            ...
            elseif dice < cycle_accum_prob.AP_delete_kink  # [DELETE_KINK]
                # println("deleting...")
                if loc ≠ _at_kink
                    @goto CYCLE_START🔁
                end
                ...
                if head.op == Ki.op # case 1 : pass kink with prob 1
                    ...
                    P_acc = inv(2 * $zhps * Wk * bond_weight(H, i, j) * update_consts.P_del2ins)
                    if metro(P_acc)
            ...
            else #if dice < cycle_accum_prob.AP_glue_worm≡1 # [GLUE_WORM]
